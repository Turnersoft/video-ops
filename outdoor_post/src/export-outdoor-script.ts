...
export async function runExportOutdoorScript(argv: string[]): Promise<void> {
  ...
  const animationPath = path.join(folder, 'animation.json');
  if (!fileExists(animationPath)) {
    throw new Error(`Missing animation.json: ${animationPath}`);
  }
  ...
  const outPath = argv[1]
    ? path.resolve(argv[1])
    : path.join(folder, 'export', `${scriptId}-outdoor-script.json`);
