---
videoOps: 1
scriptId: 02-definitions-are-apis
title: Definitions are APIs
styleKit: motion-essay
format: landscape
fps: 30
width: 1920
height: 1080
bgmProfile: motion-etude
---

# Scene 1: Definitions are APIs
<!--
layout: beat-focus
teleprompter.position: below-canvas
pace.syllables-per-second: 3.3
pace.pause-after-beat: 0.4
pace.min-beat-seconds: 5
pace.pace-factor: 1.1
-->

## Beat 1: Hook

Changing a definition breaks callers — formal math makes that visible.
### Visual notes

Opening still / title for Definitions are APIs.

## Beat 2: Develop

Expand the claim with one concrete scene the viewer can picture.
### Visual notes

Mid-episode board or still.

## Beat 3: Close

Leave one sharp takeaway — and a reason to watch the next episode.
