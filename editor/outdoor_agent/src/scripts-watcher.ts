/**
 * Watch projects/ for animation.md changes → notify listeners (live compile on read).
 * `.cache/` is written only by explicit `npm run sync` (render/export), not on every edit.
 */
...
function scriptIdFromAnimationPath(filePath: string): string | null {
  ...
  return scriptId && scriptId !== 'projects' && scriptId !== 'scripts' ? scriptId : null;
}
...
function* walkAnimationMd(): Generator<{ scriptId: string; path: string }> {
  ...
  for (const series of Deno.readDirSync(SCRIPTS_DIR)) {
    if (series.name.startsWith('.') || series.name === '_templates' || series.name === 'shared') {
      continue;
    }
    if (!series.isDirectory && !series.isSymlink) {
      continue;
    }
    const seriesDir = path.join(SCRIPTS_DIR, series.name);
    try {
      if (!Deno.statSync(seriesDir).isDirectory) {
        continue;
      }
    } catch {
      continue;
    ...
    for (const episode of Deno.readDirSync(seriesDir)) {
      if (episode.name === 'shared' || episode.name.startsWith('.')) {
        continue;
      }
      const episodeDir = path.join(seriesDir, episode.name);
      let isDir = episode.isDirectory;
      if (!isDir && episode.isSymlink) {
        try {
          isDir = Deno.statSync(episodeDir).isDirectory;
        } catch {
          isDir = false;
        }
      }
      ...
      const mdPath = path.join(episodeDir, 'animation.md');
      if (!fileExists(mdPath)) {
        continue;
      }
      let realKey = mdPath;
      try {
        realKey = Deno.realPathSync(mdPath);
      } catch {
        // keep mdPath
      }
...
/** Scan projects/ and recompile animation.md → .cache/render-props.json. */
export async function resyncAllScripts(options: { force?: boolean } = {}): Promise<{
  ...
  for (const entry of entries) {
    const cachePath = path.join(path.dirname(entry.path), '.cache', 'render-props.json');
    const needsCompile =
      options.force === true ||
      !fileExists(cachePath) ||
      mtimeMs(entry.path) > mtimeMs(cachePath) + 50;
