...
import { loadRemotionStudioUrl } from './src/remotionSettings';
...
function AppScreens() {
  ...
  const [remotionStudioUrl, setRemotionStudioUrl] = useState<string | null>(null);
  ...
  const refreshAgentUrl = useCallback(async () => {
    ...
    try {
      ...
      const [url, remotionUrl, endpoints] = await Promise.all([
        loadAgentBaseUrl(),
      ...
      setAgentBaseUrl(url);
      setRemotionStudioUrl(remotionUrl);
      ...
      const remotionUrl = await loadRemotionStudioUrl().catch(() => null);
  ...
  const seedRemotionCandidates = useMemo(() => {
    const seeds: string[] = [];
    if (remotionStudioUrl?.trim()) {
      seeds.push(remotionStudioUrl.trim().replace(/\/+$/, ''));
    }
    // Same host as agent, port 3000 — usual outdoor:all layout.
    if (agentBaseUrl?.trim()) {
      try {
        const url = new URL(agentBaseUrl);
        if (url.hostname !== '127.0.0.1' && url.hostname !== 'localhost') {
          url.port = '3000';
          seeds.push(url.origin);
        }
      } catch {
        // ignore bad agent URL
      }
    }
    return seeds;
  }, [agentBaseUrl, remotionStudioUrl]);
