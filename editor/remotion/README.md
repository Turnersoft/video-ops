...
One Remotion package renders every script. Each episode owns **`animation.md`** — the sole authoring source. `npm run sync` compiles it to `.cache/render-props.json` for Remotion Studio and renders.

## Layout

```
video_ops/
  projects/                 # all series + episodes — Remotion publicDir + sole script reservoir
  editor/remotion/          # shared Remotion project (this folder)
  manifest.json
```
...
```bash
cd video_ops/remotion
npm run sync          # compile animation.md → .cache/ for all manifest scripts
npm run sync -- --script 04-set-equality
npm run sync:force    # rebuild cache even when inputs unchanged
```
...
Output: `video_ops/projects/<series>/<slug>/export/<slug>.mp4`. MP4s are gitignored inside each script’s `export/` folder.

## Compiled cache (generated — do not edit)

Sync writes `projects/<series>/<episode>/.cache/render-props.json` and `animation-v4.json`. Remotion loads render props via `staticFile()`.
...
### Remotion shared modules (`@turn-video-shared`)

Beat-based Remotion imports editor + side-panel code from `src/shared/turn-video/` (alias `@turn-video-shared`). Knowledge and proof panels render via **turn-user** visualization modules (`@turn-user/.../visualization/knowledge`, `proof`) — not via basic_ui.
...
Knowledge and proof payloads are generated at render time via `turnSnippetKnowledgeCache` (WASM LSP) when tracks omit static JSON, or loaded from hand-authored `tracks/*.json` under each episode folder.
...
IDE tracks reference proof exports via `proofPanel.exportPath` (optional `visualizationDataPath` for embedded turn-user panel). `ProofPanelRender` uses wasm `GoalRow` + sequent rich text from LSP — do not hand-author simplified `Math` segments when using live exports.
...
3. Run `npm run sync` so `src/lib/generated/composition-meta.json` exists.
4. Restart Studio.
...
`remotion.config.ts` sets `publicDir` to `video_ops/projects/` (**not** the whole `video_ops/` tree, which would recurse `remotion/build/` into multi-GB bundles). Use `staticFile('compare/…')` — no symlink or `ensure-public` step.
