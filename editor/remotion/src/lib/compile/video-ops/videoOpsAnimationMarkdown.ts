...
import { compareHintKey, type CompareHintLayout } from '../../tracks/compareHintLayout';
...
function extractEditorComments(text: string): { stripped: string; comments: string[] } {
...
function parseFrontmatter(lines: SourceLine[]): {
    ...
    if (firstContent < 0 || lines[firstContent].text.trim() !== '---') {
        fail('animation.md must start with a --- frontmatter block.', lines[firstContent]?.number ?? 1);
    }
...
function parsePaneSection(
    ...
    assertKnownDirectives(directives, ['ref'], section.headingLine, `${side} section`);
    ...
    if (fenceStarts.length !== 0 && fenceStarts.length !== 2) {
        fail(`${side} must contain one complete fenced code block.`, section.headingLine);
    }
    ...
    const fields: Pick<VideoOpsComparePaneBeat, 'highlights' | 'goal' | 'knowledge'> = {};
    ...
    for (const line of withoutHtmlComments(contentWithoutFence)) {
        ...
        if (label) {
            ...
            activeList =
                key === 'highlights' ? 'highlights' : key === 'goal' ? 'goal' : 'knowledge';
            fields[activeList] = [];
            continue;
        }
    ...
    if (code === undefined && !ref) {
        fail(`${side} requires either a fenced code block or <!-- ref: ... -->.`, section.headingLine);
    }
    if (code !== undefined && ref) {
...
function parseHintSection(
    ...
    const target = targetRaw
        ...
        : inferredSide === 'lean'
          ...
          : inferredSide === 'turn'
            ? 'turn-code'
            : fail('Hint needs target: ... because no Lean or Turn section precedes it.', section.headingLine);
...
function parseVideoSection(section: ParsedSection): VideoOpsCompareSceneBeatV4['video'] {
    const directives = directivesFromLines(section.lines);
    assertKnownDirectives(
        directives,
        ['objectfit', 'label'],
    ...
    if (body.length !== 1) {
        fail('Video section must contain exactly one asset path line.', section.headingLine);
    }
...
function parseOverlayResource(
    ...
    const directives = directivesFromLines(lines);
    assertKnownDirectives(
        directives,
        ['type', 'src', 'objectfit', 'label', 'aataexcerpt', 'placement'],
        headingLine,
        'overlay',
    );
    if (plainLines(lines).length > 0) {
        fail('Overlay settings must be inside an HTML comment.', headingLine);
    }
    const type = directives.type?.trim().toLowerCase();
    if (type === 'textbook') {
        if (directives.placement && !['top', 'center'].includes(directives.placement)) {
            fail('Textbook overlay placement must be top or center.', headingLine);
        }
        return {
            type: 'textbook',
            ...(directives.aataexcerpt ? { aataExcerpt: directives.aataexcerpt } : {}),
    ...
    fail(`Overlay type must be textbook or video, received "${directives.type ?? ''}".`, headingLine);
}
...
function parseBeat(
    ...
    const beatWideAllowScriptChange = directivesFromLines(lines).allowscriptchange;
    ...
    for (const section of sections) {
        ...
        if (normalized === 'script' || normalized === 'say') {
            if (say) {
                fail('Put script text either before subsections or under ### Script, not both.', section.headingLine);
            }
        ...
        if (normalized === 'turn') {
            if (turn) {
                fail('A beat can only have one ### Turn section.', section.headingLine);
            }
        ...
        if (['chinese', 'say zh', 'script zh'].includes(normalized)) {
            if (sayZh !== undefined) {
                fail('A beat can only have one Chinese script section.', section.headingLine);
            }
        ...
        if (normalized === 'visual notes') {
            if (visualNotes !== undefined) {
                fail('A beat can only have one Visual notes section.', section.headingLine);
            }
        ...
        fail(`Unknown beat subsection "### ${section.heading}".`, section.headingLine);
    }
...
function parseScene(
    ...
    const firstLevelTwoOffset = levelTwoHeadings[0]?.offset ?? beatHeadings[0].offset;
    ...
    for (let headingIndex = 0; headingIndex < levelTwoHeadings.length; headingIndex += 1) {
        ...
        if (!overlayMatch) {
            fail(`Unknown scene section "## ${heading.match[1]}".`, lines[heading.offset].number);
        }
        if (hasSeenBeat) {
            fail('Overlay resources must be declared before the first beat.', lines[heading.offset].number);
        }
        const name = overlayMatch[1];
        if (overlays[name]) {
            fail(`Duplicate overlay resource "${name}".`, lines[heading.offset].number);
        }
...
function parsePositiveNumber(value: string, line: number, name: string): number {
...
function parseHintPosition(value: string, line: number): VideoOpsCompareBeatHint['layout'] {
    const values = value.split(',').map((part) => parseNumber(part.trim(), line, 'Hint position'));
...
function parsePresenter(value: string, line: number): OutdoorPresenterMode {
    ...
    if (normalized !== 'split-crop' && normalized !== 'full-clip') {
        fail(`presenter must be split-crop or full-clip, received "${value}".`, line);
    }
    return normalized;
}
...
function parsePipShape(value: string, line: number): 'circle' | 'rectangle' {
    const normalized = value.trim().toLowerCase();
    if (normalized !== 'circle' && normalized !== 'rectangle' && normalized !== 'rect') {
...
function parseNormalizedPair(
    ...
    if (parts.length !== 2) {
        fail(`${label} must be two comma-separated numbers, received "${value}".`, line);
    }
...
function parseBeatPipMask(
    ...
    return {
        shape: directives.pipshape ? parsePipShape(directives.pipshape, line) : 'rectangle',
        x: position.first,
        y: position.second,
        w: size.first,
        h: size.second,
...
function compileBeat(
    ...
    const scriptFullscreen = directives.scriptfullscreen
        ? parseBoolean(directives.scriptfullscreen, source.headingLine, 'script-fullscreen')
        : undefined;
    ...
    return {
        ...
        ...(source.visualNotes === undefined ? {} : { visualNotes: source.visualNotes }),
...
function scenePaceFromDirectives(
    ...
    return {
        ...
        ...(values.paceFactor
            ? {
                  paceFactor: parsePositiveNumber(values.paceFactor, line, 'pace.pace-factor'),
              }
            : {}),
    };
}
...
function compileScene(
    ...
    if (
        baseScene?.voiceEdit?.beatDurationsSeconds &&
        baseScene.voiceEdit.beatDurationsSeconds.length !== source.beats.length
    ) {
        fail(
            'Beat count changed but voiceEdit.beatDurationsSeconds still targets the old beat list.',
            source.headingLine,
        );
    }
    if (
        baseScene?.outdoorEdit?.beatDurationsSeconds &&
        baseScene.outdoorEdit.beatDurationsSeconds.length !== source.beats.length
    ) {
        fail(
            'Beat count changed but outdoorEdit.beatDurationsSeconds still targets the old beat list.',
            source.headingLine,
        );
    }
            ...
            ? {
                  teleprompter: {
                      ...
                      ...(position
                          ...
                          : {}),
                      ...(pace ? { pace: { ...existingTeleprompter?.pace, ...pace } } : {}),
                  },
              }
            : {}),
    ...
    const authoredComparePanes = beats.some(
        (beat) => Boolean(beat.lean || beat.turn || beat.overlay),
    );
...
function validatePane(
    ...
    const code = resolveComparePaneCode(pane, scene.compare.blocks, side, previousCode);
    for (const needle of pane.highlights ?? []) {
        if (!code.includes(needle)) {
            fail(`${side} highlight "${needle}" does not occur in the displayed code.`, line);
        }
    }
...
export function compileVideoOpsAnimationMarkdown(
    ...
    const baseSceneIndexes = options.baseAnimation?.scenes.map((scene) => scene.index) ?? [];
    ...
    if (
        ...
        (baseSceneIndexes.length !== sourceSceneIndexes.length ||
            baseSceneIndexes.some((index, offset) => index !== sourceSceneIndexes[offset]))
    ) {
        fail(
    ...
    const format = parsed.metadata.format ?? baseComposition?.format ?? 'landscape';
...
function locateMarkdownBeat(
    ...
    const nextSceneOffset = sceneHeadings[sceneArrayIndex + 1]?.offset ?? body.length;
    ...
    const nextBeatOffset = beatHeadings[beatArrayIndex + 1]?.offset ?? sceneLines.length;
    const beatEndLine =
    ...
    const sections = headingIndexes(beatBody, /^###\s+(.+?)\s*$/).map((section) => ({
...
export function updateVideoOpsAnimationMarkdownBeatEditorNotes(
    ...
    for (const section of sectionStarts) {
        if (/^(?:ai comment|comment)$/i.test(section.match[1].trim())) {
            const nextOffset =
                sectionStarts.find((candidate) => candidate.offset > section.offset)?.offset
                ?? beatBodyLines.length;
            beatBodyLines = [
    ...
    const strippedBody = extractEditorComments(beatBodyLines.join('\n')).stripped.replace(
        /\n{3,}/g,
        '\n\n',
    );
    ...
    const directiveEnd = leadingBeatDirectiveEnd(rebuiltBeatBody, 0, firstSectionOffset);
    const directiveLines = rebuiltBeatBody.slice(0, directiveEnd).map((text, offset) => ({
        number: offset + 1,
        text,
    }));
...
export function updateVideoOpsAnimationMarkdownBeatFontScales(
    ...
    for (let beatIndex = beatFontScales.length - 1; beatIndex >= 0; beatIndex -= 1) {
...
export function updateVideoOpsAnimationMarkdownHintLayouts(
    ...
    for (let beatIndex = scene.beats.length - 1; beatIndex >= 0; beatIndex -= 1) {
        ...
        for (let sectionIndex = 0; sectionIndex < located.sectionHeadings.length; sectionIndex += 1) {
            ...
            const endOffset =
                located.sectionHeadings[sectionIndex + 1]?.offset ?? located.beatEndOffset;
            parsedHints.push({
