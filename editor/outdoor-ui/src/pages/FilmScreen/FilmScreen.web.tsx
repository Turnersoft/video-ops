...
import type { LiveScript, OutdoorScript, SlideEvent, TakeManifest, TakeMarker } from '../../types';
...
// DISABLED: teleprompter countdown helper — used by runCountdown below.
...
function FilmVideoPreview({
  ...
  return (
    <View style={StyleSheet.absoluteFillObject} pointerEvents="none">
      {/* RN Web renders this DOM host for the camera preview. */}
      <video
        ref={ref}
        autoPlay
        playsInline
...
export function FilmScreen({ scriptId }: FilmScreenProps) {
  ...
  // DISABLED: teleprompter pre-roll / slide-change countdown overlay.
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setError(null);
      try {
        const live = await api.getLiveScript(scriptId).catch(async (): Promise<LiveScript> => {
          ...
          return {
            ...
            source: (outdoor.source as 'animation.md') ?? 'animation.md',
  ...
  // DISABLED: teleprompter countdown — re-enable to pause before record / slide changes.
  ...
  const goToSlide = useCallback(
    async (nextIndex: number, _withSlideCountdown: boolean) => {
      if (!script) {
        return;
      }
      ...
      // DISABLED: slide-change countdown (used nextSlide?.countdownSeconds + runCountdown)
      if (isRecording) {
        markSlide(clamped);
      }
      setSlideIndex(clamped);
    },
    [isRecording, markSlide, script, slideIndex, slides],
  );
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!activeSlide) {
      setError('Script has no slides to film.');
      return;
    }
    // DISABLED: pre-recording countdown
    ...
    chunksRef.current = [];
    slideEventsRef.current = [{ slideId: activeSlide.id, index: slideIndex, atMs: 0 }];
    markersRef.current = [];
  ...
  if (loading) {
    return renderFilmPortal(
      <View
        style={[webModuleStyle(classes.centered, classes.root), Platform.OS !== 'web' ? styles.centered : undefined]}
      >
    ...
    return renderFilmPortal(
      ...
      >
        ...
        <Text style={styles.meta}>
          Allow camera and microphone access for this site, then reload. Filming requires HTTPS or
          localhost.
        </Text>
  ...
  return renderFilmPortal(
    <View
      style={[webModuleStyle(classes.root), Platform.OS !== 'web' ? styles.root : undefined]}
    >
      ...
      {/* DISABLED: teleprompter countdown overlay
      {countdown !== null ? (
...
function renderFilmPortal(content: ReactNode) {
...
const styles = StyleSheet.create({
  ...
  promptCard: {
    ...
    borderRadius: 20,
    padding: spacing.sm,
    maxHeight: '48vh' as unknown as DimensionValue,
  },
  promptScroll: {
    maxHeight: '40vh' as unknown as DimensionValue,
  },
  promptProgress: {
