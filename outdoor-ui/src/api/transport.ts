...
async function probeHttpReachable(
  ...
  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    // Remotion may lack CORS — no-cors still fails on network errors.
    const response = await fetchImpl(normalizeUrl(url), {
      method: 'GET',
      mode: 'no-cors',
      signal: controller.signal,
    });
    clearTimeout(timer);
    // Opaque (status 0) means the host answered.
...
export async function resolveOutdoorTransport(
  ...
  const remotionOrigin = remotionWinner ?? LOCAL_REMOTION;
...
export function remotionEmbedFromOrigin(
  remotionOrigin: string,
  compositionPath: 'video-outdoor-landscape' | 'video-outdoor-portrait',
): string {
  const origin = normalizeUrl(remotionOrigin);
  return `${origin}/${compositionPath}?outdoorEmbed=1`;
}
