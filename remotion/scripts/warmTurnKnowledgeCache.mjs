 ...
 * Writes JSON under video_ops/public/turn-knowledge-cache/ for Remotion staticFile().
 *
 * Usage:
 *   node scripts/warmTurnKnowledgeCache.mjs sets-v2-01-set
...
import { resolveSharedAssetPath, scriptFolderRelative } from './scriptSeries.mjs';
...
function scriptFolder(scriptId) {
    return scriptFolderRelative(scriptId);
}
...
function cacheStaticPath(sourceFile, sourceHash) {
    ...
    return `turn-knowledge-cache/${safe}-${sourceHash}.json`;
}

function resolveTurnSourcePath(sourceFile) {
    if (sourceFile.startsWith('@turn-user/')) {
        return path.join(videoOpsDir, 'public', sourceFile.replace('@turn-user/', 'turn-user/'));
    }
...
function collectTrackRefs(scriptId) {
    const animPath = path.join(videoOpsDir, scriptFolder(scriptId), 'animation.json');
...
async function writeKnowledgeCache(sourceFile, text, label) {
    ...
    const outPath = path.join(videoOpsDir, 'public', relativeOut);
