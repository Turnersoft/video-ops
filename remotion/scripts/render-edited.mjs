...
//
// AI-voice edited export. The canonical animation.json keeps teleprompter pace
// durations (for reading along in Remotion Studio / Video Editor). This script:
//   1. reads export/voice-manifest.json (written by index-tts gen_beats.py)
//   2. temporarily injects audio-calibrated beat durations + sentence caption
//      segments + voiceEdit audio wiring into animation.json
//   3. renders via render-script.mjs
//   4. restores the canonical animation.json (always, even on failure)
//   5. renames the output to export/<slug>-edited.mp4
...
try {
    ...
    console.log('[render:edited] restored canonical animation.json (teleprompter pace)');
}
...
const defaultPath = path.join(scriptDir, 'export', `${scriptId}.mp4`);
const exportPath = path.join(scriptDir, 'export', `${scriptId}-edited.mp4`);
