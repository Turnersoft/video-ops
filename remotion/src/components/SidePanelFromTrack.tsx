...
import { TurnVideoSidePanel } from '@turn-video-shared/panels/TurnVideoSidePanel';
...
import { knowledgeDataForTurnSource } from '@turn-video-shared/ide/turnSnippetKnowledgeCache';
import { fetchVideoOpsStaticJson } from '@turn-video-shared/ide/fetchVideoOpsStatic';

import { loadIdeTrack, type TrackLoadOptions } from '../lib/loadIdeTrack';
...
import type { KnowledgePanelExport } from './KnowledgePanelRender';
import type { ProofPanelExport } from './ProofPanelRender';
    ...
    useEffect(() => {
        ...
        async function loadSidePanelData() {
            try {
                ...
                } else if (loaded.track.knowledgePanel?.generateFromSource) {
                    const beatCode = loaded.track.beatCodeSegments?.[0]?.code ?? '';
                    const analysisSource =
                        beatCode ||
                        (loaded.fullSource.length > 0 ? loaded.fullSource : loaded.renderSource);
                    if (analysisSource) {
                        setKnowledgeLoading(true);
                        const generatedKnowledgeData = await knowledgeDataForTurnSource(
                            loaded.track.sourceFile,
                            analysisSource,
