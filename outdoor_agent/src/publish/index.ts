...
import { resolveTakeFromJobId, takeStageRunDir } from '../paths.ts';
...
export function assertReadyToPublish(jobId: string): ReadyToPublish {
  ...
  const portrait = path.join(compositeDir, `${job.scriptId}-outdoor-portrait.mp4`);
  const landscape = path.join(compositeDir, `${job.scriptId}-outdoor-landscape.mp4`);
