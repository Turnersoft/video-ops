...
export function formatPipelineError(
  ...
  if (raw.includes('social-posts.json not found')) {
    return {
      ...
      message: raw,
      hint: 'Add scripts/.../social-posts.json on the Mac before running Social.',
      detail,
    };
  }

  if (raw.includes('No compare beats in animation.json')) {
    return {
      code: 'missing_file',
      title: 'Animation beats missing',
      message: raw,
      hint: 'Open animation.md in Remotion Studio and sync animation.json.',
      detail,
    };
  }
